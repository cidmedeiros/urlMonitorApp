...
handlers.checksCreate = (data,callback) => {
    ...
    if(data.method == 'get'){
      ...
      // Read in a template as a string
      tools.getTemplate('checksCreate',templateData, (err,str) => {
        if(!err && str){
          // Add the universal header and footer
          tools.addUniversalTemplates(str,templateData, (err,str) => {
            if(!err && str){
...
handlers.checksList = (data,callback) => {
    ...
    if(data.method == 'get'){
      ...
      // Read in a template as a string
      tools.getTemplate('checksList',templateData, (err,str) => {
        if(!err && str){
          // Add the universal header and footer
          tools.addUniversalTemplates(str,templateData, (err,str) => {
            if(!err && str){
...
handlers.checksEdit = (data,callback) => {
    ...
    if(data.method == 'get'){
      ...
      // Read in a template as a string
      tools.getTemplate('checksEdit',templateData, (err,str) => {
        if(!err && str){
          // Add the universal header and footer
          tools.addUniversalTemplates(str,templateData, (err,str) => {
            if(!err && str){
...
handlers._tokens.post = function(data,callback){
    ...
    if(phone && password){
        ...
        schreiber.read('users',phone,function(err,userData){
        ...
        if(!err && userData){
            ...
            var tokenId = tools.createRandomString(20);
            ...
            schreiber.create('tokens',tokenId,tokenObject,function(err){
                        ...
                        schreiber.read('users', userPhone, (err, userData) => {
                            if(!err && userData){
                                ...
                                if(userChecks.length < config.maxChecks){
                                    ...
                                    //Verify if the url given has dns entries (and therefore can resolve)
