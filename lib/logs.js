...
logSchreiber.decompress = (fileId, callback) => {
    let fileName = fileId+'.gz.b64';
    fs.readFile(logSchreiber.baseDir+fileName, 'utf-8', (err, str) => {
        if(!err && str){
            //decompress the data
...
//Truncate a log file
logSchreiber.truncate = (logId, callback) => {
    fs.truncate(logSchreiber.baseDir+logId+'.log',0, (err) =>{
        if(!err){
            callback(false);
        } else {
            callback(err);
